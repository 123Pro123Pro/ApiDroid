...
setTimeout(function() {
    Java.perform(function() {
        ...
        console.log("Please wait for 5-10 minutes for enumDexClasses from {{ apk_path }}....")
        var a = enumDexClasses("{{ apk_path }}");
        console.log("EnumDexClasses done!!!")
        a.forEach(function(s) {
            if (s) { classname_return += String(s) + '\n' }
        });
        sendback = enum_signature + classname_return
	    ...
	    var a = enumMethods(decodeURIComponent(escape(hex_to_ascii("{{ class_to_find.encode('utf-8').hex() }}"))))
	    a.forEach(function(s) { 
        ...
        // enumerate all methods in a class   
        var a = enumLibSo(decodeURIComponent(escape(hex_to_ascii("{{ class_to_find.encode('utf-8').hex() }}"))))
        a.forEach(function(s) { 
            methods_return += String(s) + '\n'
        });
        sendback = enum_signature + methods_return
